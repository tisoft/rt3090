...
#ifdef ACM_MEMORY_TEST
	printk("acm_msg> ACM_MEM_Alloc_Num = %d\n", gAcmMemAllocNum);
	printk("acm_msg> ACM_MEM_Free_Num  = %d\n", gAcmMemFreeNum);
#endif // ACM_MEMORY_TEST //
...
//		return ACM_RTN_NO_ACM;
	...
	if (pStream != NULL)
	{
		...
		TimeAllowed = pStream->pTspec->MediumTime << 5;
	...
	ACM_PARAM_IN	UCHAR					StreamType)
{
	...
	*/
	...
	pStreamReq->DialogToken = TSPEC_DIALOG_TOKEN_GET();
...
#define LMR_PREAMBL_TIME(__FlgIsGmode, __FlgIsSpreamble, __Time)	\
	...
	/* init */
	LenHeader = ACMR_FME_QOS_HEADER_SIZE;
	TxTime = 0;
	TimeRtsCts = 0;
	TxTimeCtsSelf = 0;
	TxTimeRtsCts = 0;
